...
if (proj_lib_env_undefined && proj_path && gdal.bundled) {
...
if (process.env.CURL_CA_BUNDLE === undefined) {
  const caBundle = gdal.bundled ?
    path.resolve(__dirname, '../deps/libcurl/cacert.pem') :
    findVcpkgLibPath('..') ? path.join(findVcpkgLibPath('..'), '..', '..', 'share', 'curl', 'curl-ca-bundle.crt') : null
