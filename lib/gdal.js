...
function findDataPath(subpath) {
  // Strategy 1: Conan-generated conan_data_paths.json
  const conanPathsDirs = [
    path.resolve(__dirname, '..', 'conan', 'install'),
    path.resolve(__dirname, '..', 'build', 'conan'),
    path.resolve(__dirname, '..', 'build', 'Release', 'conan'),
    path.resolve(__dirname, '..', 'build', 'Debug', 'conan')
  ]
  for (const dir of conanPathsDirs) {
    const conanFile = path.resolve(dir, 'conan_data_paths.json')
    if (fs.existsSync(conanFile)) {
      try {
        const paths = JSON.parse(fs.readFileSync(conanFile, 'utf-8'))
        const key = subpath === 'gdal' ? 'GDAL_DATA'
          : subpath === 'proj' ? 'PROJ_LIB' : null
        if (key && paths[key] && fs.existsSync(paths[key])) return paths[key]
      } catch (_) { /* fall through */ }
    }
  }

  // Strategy 2: vcpkg_installed (backward compatibility)
  // Strategy 2: vcpkg_installed (manifest mode)
  const vcpkgDirs = [
    path.resolve(__dirname, '..', 'vcpkg_installed'),
    path.resolve(__dirname, '..', 'build', 'vcpkg_installed')
  ]
  for (const installedDir of vcpkgDirs) {
...
// Resolve GDAL_DATA: env var > conan > vcpkg > bundled deps
let data_path
if (process.env.GDAL_DATA) {
  data_path = process.env.GDAL_DATA
} else {
  data_path = findDataPath('gdal')
  if (!data_path) {
...
// Resolve PROJ_LIB: env var > conan > vcpkg > bundled deps
let proj_path
if (process.env.PROJ_LIB) {
  proj_path = process.env.PROJ_LIB
} else {
  proj_path = findDataPath('proj')
  if (!proj_path) {
...
if (grib_env) {
  ...
  grib_resource_path = findDataPath('gdal') ? path.join(findDataPath('gdal'), '..', '..', 'gdal', 'frmts', 'grib', 'data') : null
...
if (!data_path) {
  throw new Error(
    'GDAL data path not found. Set GDAL_DATA environment variable or install GDAL dependencies.'
  )
}
...
if (proj_lib_env_undefined && !proj_path) {
  throw new Error(
    'PROJ data path not found. Set PROJ_LIB environment variable or install PROJ dependencies.'
  )
}
